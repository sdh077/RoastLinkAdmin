...
const getDefaultDepart = async () => {
  ...
  return await supabase.from('shop').select('depart_default').eq('user_id', session.data.user?.id).single<IDepart>()
}
...
const getDeparts = async () => {
  ...
  return supabase.from('shop_depart').select('*').eq('shop_id', shop.id).returns<IDepart[]>()
}
  ...
  return (
    <>
      <div className='grid md:grid-cols-3'>
        {departs?.map(depart =>
          <DepartCard key={depart.id} depart={depart} />
        )}
      </div>
